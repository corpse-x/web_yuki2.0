...
async def style_buttons(c, m, cb=False):
    ...
    buttons = [
        ...
        [InlineKeyboardButton ("ᴄʟᴏsᴇ",callback_data="close"),InlineKeyboardButton ("ɴᴇxᴛ ➻", callback_data="nxt")],
    ]
    if not cb:
        await m.reply_text(
...
@pbot.on_callback_query(filters.regex("^nxt1"))
async def style_buttons(c, m, cb=False):
    text = m.text.split(' ',1)[1]
    if m.data == "nxt1":
        buttons = [
...
async def nxt(c, m):
    if m.data == "nxt":
        buttons = [
            ...
            [InlineKeyboardButton ("ᴄʟᴏsᴇ",callback_data="close"),InlineKeyboardButton ("ʙᴀᴄᴋ", callback_data="nxt1")],
        ]
        await m.answer()
        await m.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
    else:
