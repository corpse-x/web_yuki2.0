...
#from config import OWNER_ID  # Import OWNER_ID from your config file
...
async def force_subscribe(event):
    ...
    if event.is_group:
        ...
        # Allow OWNER_ID or admin users
        if not perm.is_admin and event.sender_id != OWNER_ID:
            return await event.reply("You need to be an admin or the bot owner to do this.")

        # Allow only group creators or the OWNER_ID
        if not perm.is_creator and event.sender_id != OWNER_ID:
